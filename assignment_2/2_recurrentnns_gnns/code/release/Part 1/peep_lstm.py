...
class peepLSTM(nn.Module):
    ...
    def __init__(self, seq_length, input_dim, hidden_dim, num_classes,
        ...
        self.embedding = nn.Embedding(num_embeddings=3,embedding_dim=self.input_dim)
        ########################
        # END OF YOUR CODE    #
        #######################
    ...
    def forward(self, x):
        ...
        def frw_block(x,t,name): 
            # print('here')           
            out = self.sigmoid(x[:, t, :] @ self.params['W_'+name+'x'] + c_t @ self.params['W_'+name+'h'] + self.params['b_'+name])
            # if 'c' not in name:
        ...
        for t in range(self.seq_length):
            ...
            # c = frw_block(x,t,'c')
            # g = frw_block(x,t,'g')
            c_t = (self.sigmoid(x[:, t, :] @ self.params['W_'+'c'+'x'] + self.params['b_'+'c'])) *i + c_t*f
            # c_t = g*i + c_t*f
            h_t = self.tanh(c_t) * o 
